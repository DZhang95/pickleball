    ...
    std::vector<float> &radius)
{
    ...
    position.assign(3 * numCircles, 0.0f);
    velocity.assign(3 * numCircles, 0.0f);
    color.assign(3 * numCircles, 0.0f);
    radius.assign(numCircles, 0.0f);
    ...
    generateSizeCircles(numCircles, position.data(), velocity.data(), color.data(), radius.data(), BIG_RADIUS);
        int startIdx = 9 * 1000;
        int changeNum = 1 * 1000;
    changeCircles(changeNum, position.data() + startIdx * 3, radius.data() + startIdx, MICRO_RADIUS, .85f, .1f);
    ...
    generateSizeCircles(numCircles, position.data(), velocity.data(), color.data(), radius.data(), BIG_RADIUS);
        int startIdx = 9 * 1000;
        int changeNum = 1 * 1000;
    changeCircles(changeNum, position.data() + startIdx * 3, radius.data() + startIdx, MICRO_RADIUS, .05f, .1f);
    ...
    generateRandomCircles(numCircles, position.data(), velocity.data(), color.data(), radius.data());
